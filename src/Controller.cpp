...
void Controller::increaseSensitivity() {
    switch (input) {
        case mic:
            increaseMicGain();
            break;
...
void Controller::decreaseSensitivity() {
    switch (input) {
        case mic:
            decreaseMicGain();
            break;
...
void Controller::setValue(uint8_t value) {
    switch (mode) {
        case NORMAL:
            selectFx(value - 1);
            break;
...
void Controller::previousFx() {
    selectFx(stage->getPrevFxIndex(fx));
}

void Controller::nextFx() {
    selectFx(stage->getNextFxIndex(fx));
}

void Controller::randomFx() {
